...
salarios = ['250', '550', '1050', '1100']

for faixa in range(200,1001,100):
    range = int(str(faixa)[:-2])

    if faixa < 1000:
        vendedores = sum(int(salario[:-2])==range for salario in salarios)
        print(f'{vendedores} pessoa(s) recebe(m) entre {faixa} e {faixa + 99}.')
    else:
        vendedores = sum(int(salario[:-2])>=10 for salario in salarios)
        print(f'{vendedores} pessoa(s) recebe(m) mais que 1000.')