...
#Após a execução o programa deverá voltar a pedir outro valor de prestação e assim continuar até que seja informado um
#valor igual a zero para a prestação.
#Neste momento o programa deverá ser encerrado, exibindo o relatório do dia, que conterá a quantidade e o valor total
#de prestações pagas no dia.
...
def valorPagamento():
    ...
    contador = 0
    totalPrestacoesPagas = 0

    while valorPrestacao > 0:
        diasEmAtraso = int(input('Quantos dias de atraso? '))

        if diasEmAtraso <= 0:
            valorPrestacao = valorPrestacao
            contador = contador + 1
            totalPrestacoesPagas = totalPrestacoesPagas + valorPrestacao
            print('O valor a pagar é R$ {:.2f}'.format(valorPrestacao))
        else:
            valorPrestacao = valorPrestacao + (valorPrestacao * 0.03) + ((valorPrestacao * 0.01) * diasEmAtraso)
            contador = contador + 1
            totalPrestacoesPagas = totalPrestacoesPagas + valorPrestacao
            print('O valor a pagar é R$ {:.2f}'.format(valorPrestacao))

        valorPrestacao = float(input('Digite o valor da próxima prestação: '))

    print('Hoje foram pagas {} prestações com o valor total de {:.2f}'.format(contador, totalPrestacoesPagas))
valorPagamento()
